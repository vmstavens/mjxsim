...
def get_ibrl_sac_dp_config(exp_name, env, wandb):
    ...
    exploration_std = max(float(np.max(action_half_range)) * 0.5, 1e-5)
    cfg["exploration"] = {}
    cfg["exploration"]["noise"] = GaussianNoise(0, exploration_std, device=env.device)
    cfg["exploration"]["initial_scale"] = 1.0
    cfg["exploration"]["final_scale"] = 0.05
    cfg["exploration"]["timesteps"] = None
