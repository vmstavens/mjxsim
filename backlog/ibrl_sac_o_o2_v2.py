...
IBRL_SAC_DEFAULT_CONFIG = {
    ...
    "target_selection_mode": "greedy",  # use deterministic target action selection by default
}
    ...
    def _select_act(
        ...
        if self._decision_block:
            ...
            if torch.mean(target_q_il) > torch.mean(target_q_rl):
                ...
                actions = il_actions
                action_sources = torch.ones(
                    (batch_size, 1), device=il_actions.device, dtype=torch.long
                )
            else:
                actions = rl_actions
        ...
        if self._actor == "rl":
            ...
            actor_id = 1
            action_sources = torch.zeros(
                (actions.shape[0], 1), device=actions.device, dtype=torch.long
            )
        elif self._actor == "il":
            actions = il_actions
            actor_id = 2
        ...
        if not target:
            ...
            selected_log_prob = torch.zeros_like(next_log_prob)
            selected_log_prob = torch.where(
                action_sources == 0, next_log_prob, selected_log_prob
            )
            return actions, selected_log_prob, _

    def _apply_exploration_noise(
        self, actions: torch.Tensor, timestep: int, timesteps: int
    ) -> tuple[torch.Tensor, torch.Tensor]:
        if self._exploration_noise is None or not self.training:
        ...
        if self.clip_actions_min is not None and self.clip_actions_max is not None:
            actions = torch.clamp(
                actions, min=self.clip_actions_min, max=self.clip_actions_max
            )
    ...
    def _update(self, timestep: int, timesteps: int) -> None:
        ...
        for gradient_step in range(self._gradient_steps):
            ...
            if self.write_interval > 0:
                ...
                self.track_data("Target / Target Q (max)", torch.max(target_q_values).item())
                self.track_data("Target / Target Q (min)", torch.min(target_q_values).item())
                self.track_data("Target / Target Q (mean)", torch.mean(target_q_values).item())
                self.track_data("Target / Next log prob (max)", torch.max(next_log_prob).item())
                self.track_data("Target / Next log prob (min)", torch.min(next_log_prob).item())
                self.track_data("Target / Next log prob (mean)", torch.mean(next_log_prob).item())
