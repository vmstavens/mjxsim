"""JAX IBRL policy-composition shell.

This module establishes the backend namespace and callable policy surface for
JAX IBRL. Full SAC replay-buffer updates are intentionally not hidden behind a
stub; ``update`` raises until the JAX RL optimizer path is implemented.
"""
...
@dataclasses.dataclass(kw_only=True)
class IBRL_SAC_CFG:
    """Configuration for the JAX IBRL SAC shell."""
...
class IBRL:
    """JAX IBRL actor combiner.

    Args:
        policy: Reinforcement-learning actor callable ``(params, observations)``.
        imitation_policy: Imitation actor callable ``(params, observations)``.
    """
    ...
    def __init__(
        ...
        *,
        ...
        cfg: IBRL_SAC_CFG | Mapping[str, Any] | None = None,
    ):
    ...
    def act(
        ...
        *,
        ...
        if mode == "rl":
            return self.policy(self.params, obs)
        if mode == "il":
            return self.imitation_policy(self.imitation_params, obs)
        if mode != "both":
            raise ValueError("actor must be one of 'rl', 'il', or 'both'")
        mix = self.cfg.soft_update_beta if beta is None else beta
        rl_action = self.policy(self.params, obs)
        il_action = self.imitation_policy(self.imitation_params, obs)
        return mix * il_action + (1.0 - mix) * rl_action

    def update(self, batch: Any) -> None:
        del batch
        raise NotImplementedError(
            "JAX IBRL SAC replay-buffer updates are not implemented yet. "
            "Use this class for policy composition or port the SAC update loop next."
        )


def _coerce_cfg(cfg: IBRL_SAC_CFG | Mapping[str, Any] | None) -> IBRL_SAC_CFG:
    if cfg is None:
        result = IBRL_SAC_CFG()
    elif isinstance(cfg, IBRL_SAC_CFG):
        result = copy.deepcopy(cfg)
    elif isinstance(cfg, Mapping):
        result = IBRL_SAC_CFG()
        for key, value in cfg.items():
            if not hasattr(result, key):
                raise ValueError(f"Invalid IBRL config key: {key}")
            setattr(result, key, copy.deepcopy(value))
    else:
        raise TypeError(
            "cfg must be an IBRL_SAC_CFG, mapping, or None "
