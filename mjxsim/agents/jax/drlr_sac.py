"""JAX DRLR policy-composition shell."""
...
@dataclasses.dataclass(kw_only=True)
class DRLR_SAC_CFG:
    """Configuration for the JAX DRLR SAC shell."""
...
class DRLR:
    ...
    def __init__(
        ...
        *,
        ...
        cfg: DRLR_SAC_CFG | Mapping[str, Any] | None = None,
    ):
    ...
    def update(self, batch: Any) -> None:
        ...
        raise NotImplementedError(
            "JAX DRLR SAC replay-buffer updates are not implemented yet. "
            "Use this class for policy composition or port the SAC update loop next."
        )


def _coerce_cfg(cfg: DRLR_SAC_CFG | Mapping[str, Any] | None) -> DRLR_SAC_CFG:
    if cfg is None:
        result = DRLR_SAC_CFG()
    elif isinstance(cfg, DRLR_SAC_CFG):
        ...
        raise TypeError(
            "cfg must be a DRLR_SAC_CFG, mapping, or None "
