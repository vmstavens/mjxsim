...
class IBRL_SAC_CFG(AgentCfg):
    ...
    exploration: ExplorationCfg | dict[str, Any] = dataclasses.field(
        default_factory=ExplorationCfg
    )
    ...
    def expand(self) -> None:
        ...
        if isinstance(self.exploration, dict):
            self.exploration = ExplorationCfg(**self.exploration)
...
def _coerce_ibrl_sac_cfg(
    cfg: IBRL_SAC_CFG | Mapping[str, Any] | None,
) -> IBRL_SAC_CFG:
    if cfg is None:
        result = IBRL_SAC_CFG()
        ...
        for key, value in cfg.items():
            if not hasattr(result, key):
                raise ValueError(f"Invalid IBRL SAC config key: {key}")
            setattr(result, key, copy.deepcopy(value))
    else:
        raise TypeError(
            f"cfg must be an IBRL_SAC_CFG, mapping, or None (got {type(cfg).__name__})"
        )
    result.expand()
    return result
...
class IBRL(Agent):
    def __init__(
        ...
        cfg: Optional[IBRL_SAC_CFG | Mapping[str, Any]] = None,
    ) -> None:
    ...
    def init(self, trainer_cfg: Optional[Mapping[str, Any]] = None) -> None:
        if dataclasses.is_dataclass(trainer_cfg):
            trainer_cfg = dataclasses.asdict(trainer_cfg)
    ...
    def enable_training_mode(
        ...
        if apply_to_models:
            self.set_mode("train" if enabled else "eval")
