...
IBRL_SAC_DEFAULT_CONFIG = {
    ...
    "warmup_timesteps": 100_000,    # number of timesteps to conclude before mode is switched from il to both

    "il_ctrl_scale": 0.3,           # since il needs traj, sends to overshoot, this is to prevent that 
    "rl_il_equal_scale": True,      # scale the rl actions to be of same length as il actions
    "dctrl_rl": [-0.01, 0.01],      # the control limits of the RL agent
...
class IBRL(Agent):
    def __init__(
        ...
        self._rl_il_equal_scale = self.cfg["rl_il_equal_scale"]
        self._dctrl_rl: torch.Tensor = torch.tensor(self.cfg["dctrl_rl"]).to(
            self.device
        )
    ...
    def _select_act(
        ...
        # scale RL action, assuming RL gives [-1, 1]
        low, high = self._dctrl_rl
        rl_actions = (rl_actions + 1) * 0.5 * (high - low) + low
        ...
        # Align with DP rollout: take action at obs_horizon - 1
        # obs_horizon = getattr(self.IL_policy, "_obs_horizon", 1)
        # action_index = max(obs_horizon - 1, 0)
        action_index = 0
        ...
        # print(f"{il_actions.shape=}")

        # il_actions=tensor([[1.1345e-04, 6.7856e-06, 1.7995e-03]], device='cuda:0')
        # rl_actions=tensor([[ 0.1265, -0.1340, -0.0674]], device='cuda:0')

        # scale il_actions
        il_actions = il_actions * self._il_ctrl_scale

        if self._rl_il_equal_scale:
            il_action_length = torch.linalg.norm(il_actions)
            normalized_rl_actions = rl_actions / torch.linalg.norm(rl_actions)
            rl_actions = normalized_rl_actions * il_action_length

        # Change this line - instead of concatenating, stack the actions
        # this leaves (   )
        rl_il_actions = torch.stack([rl_actions, il_actions], dim=1)
        # print(f"{rl_il_actions.shape=}")
        ...
        if self._decision_block:
            ...
            if soft:
                ...
                if self._actor == "rl":
                    self.track_data("ActionSelection / il_selection_ratio", 0.0)
                elif self._actor == "il":
                    self.track_data("ActionSelection / il_selection_ratio", 1.0)
                elif self._actor == "both":
                    self.track_data("ActionSelection / il_selection_ratio", pct_of_il)
                else:
                    raise ValueError(
    ...
    def act(self, states: torch.Tensor, timestep: int, timesteps: int) -> torch.Tensor:
        ...
        # if timestep > 2000:
        #     with open("testing/experiments/pipe_insert/tmp/ibrl_data.json", "w") as f:
        ...
        # SAVE_STATE["actions"].append(actions.tolist())
        # SAVE_STATE["states"].append(il_states.tolist())
